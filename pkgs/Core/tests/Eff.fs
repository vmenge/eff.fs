        ...
        testList "Core" [
            ...
            testTask "mapErr maps bound errors" {
                ...
                Expect.equal
                    err.Message
                    "boom"
                    "should map Err values after bind"
            }
            ...
            testTask "Task resolves" {
                let! value =
                    Eff.ofTask (fun () -> task { return 5 }) |> Eff.runTask ()
            ...
            testTask "ValueTask resolves" {
                let! value =
                    Eff.ofValueTask (fun () -> ValueTask<int>(5))
                    |> Eff.runTask ()
            ...
            testTask "Result resolves" {
                ...
                let! value =
                    Eff.ofResultWith exn (Error "oh yes") |> Eff.runTask ()
            ...
            testTask "Option resolves" {
                ...
                let! value =
                    Eff.ofOptionWith (fun () -> exn "missing") None
                    |> Eff.runTask ()
            ...
            testTask "exceptions resolve as defects" {
                let! value =
                    Eff.thunk (fun () -> failwith "oh no") |> Eff.runTask ()
            ...
            testTask "env resolves" {
                ...
                let! value =
                    Eff.read (fun (e: {| UserId: int; Name: string |}) ->
                        e.UserId
                    )
                ...
                Expect.equal
                    value
                    (Exit.Ok "43")
                    "should project UserId from env"
            }
            ...
            testTask "defer runs after tryCatch failure" {
                ...
                Expect.equal
                    err.Message
                    "boom"
                    "should preserve the captured exception"
